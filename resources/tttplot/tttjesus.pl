...
#
	...
	print GPL_LIN_FILE <<EOF;
		...
		plot "$emp_lin_filename" t "empirical" w points, "$the_lin_filename" t "estimated" with lines, "$up_lin_filename" t "+1 std dev range" w lines, "$lo_lin_filename" t "-1 std dev range" w lines 
		quit
EOF
...
#
	...
	print GPL_EXP_FILE <<EOF;
		...
		plot "$emp_exp_filename" t "empirical" w points, "$the_exp_filename" t "theoretical" w lines
		quit
EOF
